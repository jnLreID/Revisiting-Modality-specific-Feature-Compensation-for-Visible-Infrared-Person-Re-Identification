...
from model.s1model import Generatorweight
from utils import *
import time 
...
if dataset =='regdb':
    ...
    for trial in range(10):
        ...
        gall_feat = gall_feat.detach()
        distmat = re_ranking(query_feat,gall_feat,k1=31, k2=19, lambda_value=0.3) 
        # best-distmat = re_ranking(query_feat,gall_feat,k1=30, k2=19, lambda_value=0.3) 
        # fc feature 
        # distmat = np.matmul(query_feat, np.transpose(gall_feat))
        cmc, mAP  = eval_sysu(distmat, query_label, gall_label,query_cam, gall_cam)
        # pool5 feature
        # query_feat_pool = torch.tensor(query_feat_pool)
        # gall_feat_pool = torch.tensor(gall_feat_pool)
        # distmat_pool = re_ranking(query_feat_pool,gall_feat_pool,k1=20, k2=6, lambda_value=0.3)
        # distmat_pool = np.matmul(query_feat_pool, np.transpose(gall_feat_pool))
